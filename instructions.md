...
## We arent using the FFTW properly so we need to compile again 
Fist we clean:
```bash
rm -rf $PROJECT/cp2k-gpu-build/build
rm -rf $PROJECT/cp2k-gpu-install
```

```bash
#!/bin/bash

# === Load CP2K GPU build environment ===
...
# CP2K environment setup (only if on JURECA)
if [[ "$(hostname)" == jrc* ]]; then
  export LD_LIBRARY_PATH=$PROJECT/cp2k-gpu-install/lib:$LD_LIBRARY_PATH
  export LD_LIBRARY_PATH=$PROJECT/libxc-7-install/lib64:$LD_LIBRARY_PATH
  export LD_LIBRARY_PATH=$PROJECT/dbcsr-install/lib64:$LD_LIBRARY_PATH
  export LD_LIBRARY_PATH=/p/software/default/stages/2025/software/libxsmm/1.17-GCC-13.3.0/lib:$LD_LIBRARY_PATH
  export LD_LIBRARY_PATH=/p/software/default/stages/2025/software/imkl/2024.2.0/lib:$LD_LIBRARY_PATH
  export CPATH=/p/software/default/stages/2025/software/FFTW.MPI/3.3.10-gompi-2024a/include:$CPATH
  export LIBRARY_PATH=/p/software/default/stages/2025/software/FFTW.MPI/3.3.10-gompi-2024a/lib:$LIBRARY_PATH
  export LD_LIBRARY_PATH=/p/software/default/stages/2025/software/FFTW.MPI/3.3.10-gompi-2024a/lib:$LD_LIBRARY_PATH
  echo "we are in GPU: $(hostname)"
fi
...
cmake ../cp2k \
  ...
  -DDBCSR_ACC=CUDA \
  -DDBCSR_CUDA_ARCH="80" \
  -DCMAKE_INSTALL_PREFIX=$CP2K_INSTALL \
  -DCMAKE_PREFIX_PATH="/p/software/default/stages/2025/software/FFTW.MPI/3.3.10-gompi-2024a;$PROJECT/libxc-7-install;$PROJECT/dbcsr-install" \
  -DCMAKE_Fortran_COMPILER=$(which mpifort) \
  -DCMAKE_EXE_LINKER_FLAGS="-lxsmm -lmkl_rt"
...
# === a simple check ===
$PROJECT/cp2k-gpu-install/bin/cp2k.psmp --version
